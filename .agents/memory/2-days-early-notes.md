...
## Image performance: no optimizer, so size source assets by hand
`next.config.js` sets `images: { unoptimized: true }` and every image is a plain `<img>` (no `next/image`), so whatever ships in `public/images` is served at full byte size — oversized source exports are the dominant initial-load cost here, not code.
- **Rule:** size assets to their display box (≈2x for retina; ~600px max dimension covers the partner cards). Add `loading="lazy"` + `decoding="async"` to below-the-fold imgs (partner/portfolio cards); `loading="eager"` + `fetchPriority="high"` on the hero logo.
- Resize recipe (only `magick` is available — no pngquant/cwebp): `magick in.png -resize 600x600\> -strip -define png:compression-level=9 out.png` preserves transparency on the `-nobg` PNGs. Transparent **photos** stay heavy as PNG; WebP/AVIF would cut more but needs reference changes.
**Why:** a "slow load" report here was almost entirely oversized PNGs; the dev preview's one-time first-request compile is separate and never affects production.
