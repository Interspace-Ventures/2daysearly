...
const CompanyCard = memo(({ company }: CompanyCardProps) => {
  console.log(`[CARD] Rendering ${company.name} with logo path: ${company.logo}`);
  ...
  return (
    ...
    >
      ...
      <div className="h-48 w-full flex items-center justify-center p-8 bg-yellow-100 border border-red-500">
        <div className="text-xs mb-2 text-red-600">Path: {company.logo}</div>
        <img
          src={company.logo}
          alt={`${company.name} logo`}
          className="max-w-full max-h-full object-contain transition-opacity duration-300 group-hover:opacity-20 border-2 border-blue-500"
          onLoad={() => console.log(`[SUCCESS] ${company.name} logo loaded`)}
          onError={(e) => {
            console.log(`[ERROR] ${company.name} logo failed to load from ${company.logo}`);
